...
⭐ **Three of the five have PROBE CODE THAT ALREADY WORKS**, in `W:\temp\claude\slice41`,
...
atmosphere, and HALF of what was missing is now DONE: ρ(z) reached the COUPLED airframe path only, and since the
2026-09-06 bug fix it reaches `:six_dof` too (that half was never a deferral — it was an unread key, see the
LAUNCH ALTITUDE row above). What REMAINS deferred: the point-mass/ballistic drag path keeps a constant ρ
